...
def test(us_device, dataset):
    with torch.no_grad():
        for idx, (gt, pose) in enumerate(dataset):
            pred = us_device(pose)
            img1 = gt[us_device.subsample_slice].squeeze().cpu().numpy()
            img2 = pred.cpu().numpy()
            save_side_by_side_image(img1, img2, f'/raid/liujie/code_recon/code/ultra-nerf-pytorch/output/SyentheticLiver/{idx}.png')
...
def main():
    ...
    test_dataset = SyentheticDataset(liver_path) 
    trained_model_path = '/raid/liujie/code_recon/code/ultra-nerf-pytorch/nerf_30.pt'
    us_device = SyentheticLiverProbe(trained_model_path) # Note: important to ignore first several rows in the first epoch for synthetic liver dataset
