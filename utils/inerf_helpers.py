# This file comes from: https://github.com/salykova/inerf
import torch
...
class camera_transf_translation_only(nn.Module):
    def __init__(self):
        super().__init__()
        self.d = nn.Parameter(torch.normal(0., 1e-6, size=(3,)))

    def forward(self, x):
        t_0 = torch.eye(4)
        t_0[:3, -1] = self.d
        T_i = torch.matmul(t_0, x)
        return T_i
